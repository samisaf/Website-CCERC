...
The publications list is automatically generated by searching PubMed for articles authored by the consortium members. To update the list, follow these two steps:

1.  **Fetch PubMed results**:
    ```bash
    npm run fetch-pubmed
    ```
    This script searches for articles from all members specified in `content/members/*.md` and saves the data to `public/pubmed-results.json`.

2.  **Update publications page**:
    ```bash
    npm run update-publications
    ```
    This script processes the saved JSON data, deduplicates entries, sorts them by year, and updates the `content/publications.md` file.

Note that `npm run build` is configured to automatically run both of these scripts before building the site.