...
The publications list is automatically updated during the build process if the `fetchPubmed` and `updatePublications` flags are set to `true` in the frontmatter of `content/publications.md`.

When the site is built (e.g., via `npm run build`), the following steps occur automatically:
1.  **PubMed search**: The build process searches for articles from all members specified in `content/members/*.md`.
2.  **Data processing**: Results are deduplicated, sorted by year, and used to update `content/publications.md`.

You can also run these steps manually if needed, although it is usually not necessary:
```bash
# To fetch and update without a full build
