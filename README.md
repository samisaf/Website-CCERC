...
1.  **Fetch PubMed results**:
    ...
    ```
    This script searches for articles from all members specified in `content/members/*.md` and saves the data to `scripts/pubmed-results.json`.
...
These scripts should be run periodically to keep the website up to date with the latest research.